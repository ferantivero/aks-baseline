...
1. Create the AKS jump box image builder network spoke.
   ...
   az deployment group create -g rg-enterprise-networking-spokes-${LOCATION_AKS_BASELINE} -f networking/spoke-BU0001A0008-00.bicep -p hubVnetResourceId="${RESOURCEID_VNET_HUB}"
   ```
...
1. Update the regional hub deployment to account for the requirements of the spoke.
   ...
   ```bash
   RESOURCEID_SUBNET_AIB=$(az deployment group show -g rg-enterprise-networking-spokes-${LOCATION_AKS_BASELINE} -n spoke-BU0001A0008-00 --query properties.outputs.imageBuilderSubnetResourceId.value -o tsv)

   # [This takes about five minutes to run.]
   az deployment group create -g rg-enterprise-networking-hubs-${LOCATION_AKS_BASELINE} -f networking/hub-regionA.v2.bicep -p aksImageBuilderSubnetResourceId="${RESOURCEID_SUBNET_AIB}"
   ```
